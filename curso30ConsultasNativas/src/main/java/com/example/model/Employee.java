...
/*Consulta o query nombrada. Nos creamos esta consulta en la entity y luego la podemos llamar cuando queramos hacer
* una consulta a la base de datos. (Ver test "namedNativeTuple")*/
@NamedNativeQuery(
        name = "find_employee_emails",
        query = "SELECT email FROM employees"
)
/*Esta consulta nombrada esta haciendo uso de la clase record EmployeeSalary, ¿Como? A través de ->
employee_salary_mapping -> No existe en este momento, asi que hay que crearla (justo debajo)
(ver test "namedNativeDTO")*/
@NamedNativeQuery(
        name = "find_employee_salaries",
        query = "SELECT e.id, e.salary, e.bonus FROM employees e",
        resultSetMapping = "employee_salary_mapping"
)
/*Enlaza con la clase EmployeeSalary y le dice el tipo de cada campo, para el mapeo*/
@SqlResultSetMapping(
